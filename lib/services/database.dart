...
class DatabaseService {
  ...
  Future<List<Brewery>> getBrewerys() async {
    ...
    await firestoreInstance.collection("beer").getDocuments().then((querySnapshot) {
      querySnapshot.documents.forEach((result) {
        brewery.add(new Brewery(id: result.documentID, price: result['price'], dir: result['dir'], logo: result['logo']));
      });
    });
    return brewery;
  }
    ...
    List<Beer> beerList = new List();
         await firestoreInstance
            ...
            querySnapshot.documents.forEach((result){
              ...
              beerList.add(new Beer(id: result.documentID, name: beerJson['name'], origin: beerJson['origin'], type: beerJson['type'], url: beerJson['url']));
          });
        });
    return beerList;
  }
  ...
  Future<List<Categoria>> getCategoria() async {
    ...
    await firestoreInstance.collection("categoria").getDocuments().then((querySnapshot) {
      querySnapshot.documents.forEach((result) {
        categoria.add(new Categoria(id: result.documentID, name: result['name']));
      });
    });
    return categoria;
  }
  ...
  Future<List<FiltroCategoria>> getFiltroCategoria(String name) async { //recuperar subcollection
    ...
    await firestoreInstance
        ...
        .then((querySnapshot) {
      querySnapshot.documents.forEach((result){
        filtroList.add(new FiltroCategoria(id: result.documentID, name: result['name']));
      });
    });
    return filtroList;
  }
  ...
  Future<List<BeerCategoria>> getBeerCategoria() async {
    ...
    await firestoreInstance.collection("beercategoria").getDocuments().then((querySnapshot) {
      querySnapshot.documents.forEach((result) {
        beerCategoria.add(new BeerCategoria(id: result.documentID, name: result['name']));
      });
    });
